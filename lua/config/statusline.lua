...
for name, colors in pairs(mode_colors) do
    local group = "StatuslineMode" .. name:gsub("^%l", string.upper)
    vim.api.nvim_set_hl(0, group, {
        fg = colors.fg,
        bg = colors.bg,
        bold = true,
    })
    hl_groups[name] = group
end
...
local function current_mode()
    ...
    local entry = mode_map[code]
    if not entry then
        return "%#StatuslineModeNormal# " .. code:upper() .. " %*"
    end
    local label = entry[1]
    local color = entry[2]
    return "%#" .. hl_groups[color] .. "# " .. label .. " %* "
end
...
local function git_branch()
    ...
    if head and head ~= "" then
        return " %#StatuslineGitBranch#  " .. head .. " %*"
    end
    return ""
end
